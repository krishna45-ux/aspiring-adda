...
const SoftSkills: React.FC = () => {
  ...
  const analyzeAssessment = async () => {
    ...
    try {
        ...
        setAssessmentFeedback(result.text || "Could not analyze at this time.");
    } catch (e) {
        console.error(e);
  ...
  const handleAICoach = async () => {
    ...
    try {
        ...
        const result = await response.json();
        setAiResponse(result.text || "I couldn't generate a response at this time.");
    } catch (error) {
        console.error(error);
            ...
            <div className={`card-base p-6 md:p-8 mb-8 ${isDark ? 'bg-white/5' : 'bg-white'}`}>
                ...
                <ul className="space-y-3">
                    {selectedSkill.tips.map(tip => (
                        <li key={tip} className="flex gap-3 text-zinc-600 font-bold dark:text-gray-400 dark:font-normal text-sm md:text-base"><span className="text-amber-500">•</span>{tip}</li>
                    ))}
                </ul>
            </div>
                    ...
                    <div className={`card-base w-full max-w-2xl overflow-hidden flex flex-col max-h-[85vh] ${isDark ? 'bg-[#0a0a0a]' : 'bg-white'}`}>
                        <div className={`p-4 md:p-6 border-b-2 border-black dark:border-white/10 flex justify-between items-center ${isDark ? 'bg-white/5' : 'bg-neo-purple'}`}>
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="font-black text-black dark:text-white uppercase font-brutal dark:font-sans text-sm md:text-base">AI Coach</h3>
                                </div>
                            </div>
                        ...
                        <div className="p-4 border-t-2 border-black dark:border-white/10 bg-white dark:bg-[#0a0a0a]">
                            <div className="relative">
                                <textarea 
                                    ...
                                    className="w-full pl-4 pr-12 py-3 bg-zinc-100 border-2 border-black rounded-xl focus:bg-white outline-none resize-none font-bold text-black dark:bg-white/5 dark:border-white/10 dark:text-white dark:focus:border-indigo-500 text-sm"
                                    rows={2}
                                />
                                <button 
